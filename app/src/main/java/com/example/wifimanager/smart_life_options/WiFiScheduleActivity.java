...
public class WiFiScheduleActivity extends AppCompatActivity {

    private TextView repeatValueTextView;
    private TextView wifiOffTimeTextView;
    private TextView wifiOnTimeTextView;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_wi_fi_schedule);

        repeatValueTextView = findViewById(R.id.repeat_value);
        wifiOffTimeTextView = findViewById(R.id.wifi_off_time);
        wifiOnTimeTextView = findViewById(R.id.wifi_on_time);
        Button okButton = findViewById(R.id.ok_button);
        View backButton = findViewById(R.id.back_button);

        findViewById(R.id.repeat_option).setOnClickListener(v -> showRepeatOptionsDialog());
        findViewById(R.id.wifi_off_option).setOnClickListener(v -> showTimePickerDialog(true));
        findViewById(R.id.wifi_on_option).setOnClickListener(v -> showTimePickerDialog(false));
    ...
    private void showTimePickerDialog(boolean isOffTime) {
        ...
        TimePickerDialog timePickerDialog = new TimePickerDialog(
                ...
                (view, hourOfDay, selectedMinute) -> {
                    ...
                    if (isOffTime) wifiOffTimeTextView.setText(formattedTime);
                    else wifiOnTimeTextView.setText(formattedTime);
                },
    ...
    private void saveScheduleAndFinish() {
        String offTime = wifiOffTimeTextView.getText().toString();
        String onTime = wifiOnTimeTextView.getText().toString();

        if (offTime.equals(onTime)) {
            Toast.makeText(this, "Turn off and turn on times cannot be the same", Toast.LENGTH_SHORT).show();
            return;
        }

        Intent resultIntent = new Intent();
        resultIntent.putExtra("repeat_mode", currentRepeatMode);
        resultIntent.putExtra("off_time", offTime);
        resultIntent.putExtra("on_time", onTime);
